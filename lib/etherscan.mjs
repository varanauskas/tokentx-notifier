...
export async function getTokenTx(etherscan, params, attempt = 1) {
    ...
    if (!response.ok) {
        if (response.status >= 500 && attempt <= 1024) {
            console.warn(`${etherscan} got ${response.status}, retrying after ${attempt} ms`);
    ...
    if (json.status !== "1") {
        if (json.result === "Invalid API Key" && attempt <= 1024) {
            console.warn(`${etherscan} got ${json.result}, retrying after ${attempt} ms`);
            await sleep(attempt * 750);
            return getTokenTx(etherscan, params, attempt * 2);
        }

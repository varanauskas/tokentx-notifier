...
export const formatNewTxs = (txs, etherscan) => `🚨 New transactions in ${formatEtherscan(etherscan)}:

${txs.map(tx => formatTx(tx, etherscan)).join('\n\n')}`;

export const formatStatus = (watch, pkg) => `⏰ [${escape(pkg.name)}](${escape(pkg.homepage)}) \\(v${escape(pkg.version)}\\) is listening for new token transactions on these addresses:
